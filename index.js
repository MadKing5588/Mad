...
⭐✑  ${metadata.subject} منور في

⭐✑ ملاحظات الجروب : ${metadata.desc}

⭐✑ مرحبا بمن أنار الجروب شوقا، صل علي سيدنا محمد 💞، نتمني منك الإحترام والسعادة في الدنيا والآخرة`} )
                } else if (anu.action == 'remove') {
                    GojoMdNx.sendMessage(anu.id, { image: Tol, contextInfo: { mentionedJid: [num] }, caption: `⭐✑ @${num.split("@")[0]} Left ${metadata.subject}

⭐✑ لا تدوم السعادة، ولا يدوم الفرح.  نحزن لإعلان مغادرة هذا العضو الرائع من مجتمع الكلاونات خاصتنا 😢` })
                }
            }
        } catch (err) {
            console.log(err)
